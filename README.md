...
- **Deliberate removal of the `duration` feature.** Call duration is only known *after* the call ends — using it would be target leakage. Many public solutions to this dataset keep it and report inflated results; this project excludes it for a realistic pre-call prediction scenario.
